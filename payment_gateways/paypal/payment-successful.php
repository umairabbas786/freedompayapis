<?php
    ...
    $sql = "insert into deposits(user_id,currency_id,payment_method_id,uuid,charge_percentage,amount,status,created_at,updated_at) values('$user_id',1,3,'$uuid','$fees','$amount','Success',now(),now())";
    $r = $conn->query($sql);
    ...
    $sql = "insert into transactions(user_id,currency_id,payment_method_id,uuid,transaction_reference_id,transaction_type_id,user_type,subtotal,percentage,charge_percentage,total,status,created_at,updated_at)
            values('$user_id',1,3,'$uuid','$last_id',1,'registered','$amount','$charge_percentage','$fees','$amount_with_fee','Success',now(),now())";
    $r = $conn->query($sql);
